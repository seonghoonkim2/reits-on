...
function aboutPage() {
  ...
  <div class="card">
    ...
    <p class="small">현재가 ÷ 장부상 주당순자산(자본총계 ÷ 발행주식수). 시가 재평가가 아닌 장부 기준이며, 1배 미만이면 장부 대비 할인입니다. 발행주식수는 <b>배당총액÷주당배당</b>(확정 공시 우선)으로 여러 시점을 <b>교차검증</b>해, 시점 간 7% 넘게 어긋나면(유상증자 등으로 순자산 시점과 주식수 시점이 불일치) <b>"산정중"으로 비웁니다</b>. 값을 지어내지 않습니다.</p>
    <h3>배당 캘린더 (예상 + 확정)</h3>
    <p class="small">기본은 결산월 기준의 <b>예상</b> 배당기준월입니다. 여기에 더해 <b>배당결정 공시(DART)가 뜨면 자동으로 파싱</b>해 1주당 배당금·배당기준일·지급 예정을 <b>확정</b>으로 표시합니다(종목 페이지 초록 박스·캘린더 단발 일정). 공시 원문 링크를 항상 함께 제공하며, 파싱 실패 시 표시하지 않습니다.</p>
    <h3>갱신 주기</h3>
